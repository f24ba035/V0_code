import { CheckCircle, Camera, Settings, Video, PlayCircle } from "lucide-react"
import React from "react"

export default function YoutubeOperationFlow() {
  const flowSteps = [
    {
      icon: <CheckCircle className="w-12 h-12 text-[#990001]" />,
      title: "ヒアリングMTG",
      description: "事前すり合わせのために行わせていただきます。",
    },
    {
      icon: <Camera className="w-12 h-12 text-[#990001]" />,
      title: "撮影",
      description: "指定した日程で、訪問させていただきます。",
      note: "※切り抜きプランは撮影を実施しません。",
    },
    {
      icon: <Settings className="w-12 h-12 text-[#990001]" />,
      title: "初期設計",
      description: "運用に必要な投稿作成などの初期構築を行わせていただきます。",
    },
    {
      icon: <Video className="w-12 h-12 text-[#990001]" />,
      title: "動画編集",
      description: "動画編集のベースを作りながらお客様のご要望に合わせて、作り上げていきます。",
    },
    {
      icon: <PlayCircle className="w-12 h-12 text-[#990001]" />,
      title: "運用開始",
      description: "運用開始後はご不明点などはLINEグループにていつでもご連絡ください。",
    },
  ]

  return (
    <section id="youtube-operation-flow" className="py-16 md:py-24 bg-white">
